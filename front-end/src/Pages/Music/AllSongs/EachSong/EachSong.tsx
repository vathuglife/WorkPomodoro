...
import './EachSong.css'
import { SongInfo } from "../AllSongs";

interface EachSongProps{    
    songInfo:SongInfo;
    index:number;
}
export default function EachSong ({songInfo,index}:EachSongProps){
    ...
    const menuList:MenuItem[] = [
        {icon:plusIcon,label:'Add to Playlist',color:'#000034',function:doSomeShit},
        {icon:trashIcon,label:'Delete',color:'red',function:doSomeShit}
    ]
    return(
        <li className='list-item'>
            <div className='index'>{index+1}</div>
            <img className='thumbnail' src={songInfo.thumbnail}></img>
            <div className='song-info'>
                <div className='song-title'>{songInfo.title}</div>
                <div className='song-duration'>{songInfo.duration}</div>
            </div>     
